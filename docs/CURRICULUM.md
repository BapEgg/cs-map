...
4. ⬜ Java/JVM
5. ⬜ Spring/JPA
6. ⬜ 서비스 설계 (마지막에 "예매·주문 요청 하나가 끝까지 처리되는 과정")
7. ⬜ 운영체제 나머지(스케줄링·가상 메모리 보강, 파일·I/O), 컴퓨터 구조 보강
8. ⬜ 자료구조·알고리즘 — 다른 단원이 선수 개념으로 필요로 하는 것부터(해시 → 인덱스, 트리 → B-tree, 큐 → MQ·스레드 풀)
...
| index | 인덱스 | ⬜ | tree(B-tree), disk | 클러스터드/논클러스터드, 복합 인덱스, 커버링 |
...
| java-thread-pool | 스레드 풀(Executor) | ⬜ | thread-pool | thread-pool의 Java 쪽 — 하나로 합칠지 검토 |
...
## 8. 서비스 설계 `service-dev/design` (신규 폴더)
