...
ORM이 "연관은 접근할 때 가져온다"(지연 로딩)를 기본으로 택한 것은 필요 없는 것을 미리 가져오지 않기 위해서였다. 그 선택이 반복문과 만나면 행마다 쿼리가 된다. 즉 N+1은 버그가 아니라 지연 로딩의 자연스러운 결과이고, "이 화면에서 무엇이 필요한지"를 쿼리에 미리 알려 주는 것이 해결이다. 반대로 전부 EAGER로 두면 안 쓰는 것까지 항상 조인하고, 컬렉션이 둘 이상이면 곱집합이 생긴다.
...
- N+1을 EAGER로 고치지 않는다 — 모든 조회에 조인이 붙고 다른 N+1이 생긴다.
